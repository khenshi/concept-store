...
- `phone`: trimmed, 7–30 characters, and validated as a Philippine mobile or
  landline number while preserving user-readable formatting.
...
- Creation uses a directory modal with debounced live field validation.
...
Implemented and verified in reviewable parts. Later plans must not treat this
archived document as authorization for additional merchant behavior.
