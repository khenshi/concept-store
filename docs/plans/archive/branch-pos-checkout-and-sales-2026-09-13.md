...
**Status:** Completed; all seven parts reviewed and approved.
**Date:** September 13, 2026

This is a historical implementation record, not authorization for further work.
The user approved Part 7 on September 13, 2026. Its reviewed implementation and
this archive are committed together; the commit is recorded in Git history.
Earlier checkpoint instructions below describe the historical delivery workflow.
...
### Part 7 delivery — September 13, 2026

Part 6 was approved and committed as `1470369`. Part 7 implements staff branch sales
history/receipt reads and merchant own-sale history/details with historical selling-
branch discovery. Thin routes delegate to scoped sales components. Merchant sidebar
and overview Sales entries do not widen general branch/address/catalog permissions.
Staff branch details and POS link to history and saved receipt detail. Strict reduced
merchant contracts reject full staff receipt fields, and exact own-item amounts remain
integer-cent strings. Bounded pagination and From-inclusive/Until-exclusive UTC filters
reset page on range changes. Read retries clear stale snapshots/print controls; old
user/org/role/branch/sale responses cannot restore previous data. No aggregate reports,
sale mutations, refunds, gateways or excluded features are added.

The user explicitly waived rendered responsive, keyboard/dialog, zoom and print QA
for this POS/sales milestone on September 13, 2026. This new waiver does not claim
rendered certification or inherit a previous milestone's waiver.

Stop for final Part 7 review before commit. Archive this implementation plan after
final approval; no next implementation is authorized without a new current plan.
Unrelated root package/audit changes remain untouched.

Final verification passes: Prisma format/validate/generate; backend format/lint/build,
258 unit tests, 106 HTTP tests and 99 PostgreSQL integration tests, with the final
database suite passing twice consecutively; frontend format/typecheck/lint, 401 tests
across 63 files and production build; diff whitespace checks. Merchant tests cover
forbidden fields, stale-role receipts, precise maximum-capacity own subtotals,
historical branches, scoped reads, pagination/UTC filters and obsolete responses.
Staff print/refresh tests perform reads/printing only. All five migrations and the
destructive demo seed were verified only in the disposable PostgreSQL 17 container.
Its seed has three completed sales, zero incorrect sale totals and zero unbalanced
inventory ledgers, with expected independent branch prices/balances. No application
database was migrated/reset. Existing multiple-lockfile build and Node VM-modules
integration warnings remain outside feature scope.

Rendered responsive, keyboard/dialog, zoom and print checks were waived, not performed;
automated verification does not certify browser rendering or actual printer output.
